...
int main(int argc, char **argv)
{
    ...
    unsigned long nSize = 1395864372;  //c.a. 1,3 * 4 GB
...
/*********** on HOST *******************/
    ...
    gpuResult2 = (float*)malloc(nBytes);
...
/********** ON GPU **********************/
    ...
    CHECK(cudaMallocManaged(&d_A, nBytes));
    CHECK(cudaMallocManaged(&d_B, nBytes));
    //CHECK(cudaMalloc((float**)&d_Aa, nBytes));
    //CHECK(cudaMalloc((float**)&d_Bb, nBytes));
    //CHECK(cudaMalloc((float**)&d_Cc, nBytes));
    CHECK(cudaMallocManaged(&d_Cc, nBytes));
    CHECK(cudaMallocManaged(&d_Aa, nBytes));
    CHECK(cudaMallocManaged(&d_Bb, nBytes));

    initialData(d_Aa, nSize);
    initialData(d_Bb, nSize);
    initialData(d_A, nSize);
    initialData(d_B, nSize);

    // Data transfer : Host --> Device
    //CHECK(cudaMemcpy(d_A, h_A, nBytes, cudaMemcpyHostToDevice));
    //CHECK(cudaMemcpy(d_B, h_B, nBytes, cudaMemcpyHostToDevice));
    //CHECK(cudaMemcpy(d_Aa, h_A, nBytes, cudaMemcpyHostToDevice));
    //CHECK(cudaMemcpy(d_Bb, h_B, nBytes, cudaMemcpyHostToDevice));
    ...
    AddVecOnGPU<<<grid, block>>>(d_Aa, d_Bb, d_Cc, nSize);	    
...
//  printf("GPU Timer : %f ms , CPU Timer : %f ms\n",Etime, ElapsedTime*1000.0);
    ...
    CHECK(cudaMemcpy(gpuResult2, d_Cc, nBytes, cudaMemcpyDeviceToHost));
/****************************************/

    // check results
    checkResult(hostResult, gpuResult1, nSize);
    checkResult(hostResult, gpuResult2, nSize);

    // memory deallocate
    free(h_A),      free(h_B),      free(hostResult),       free(gpuResult1);
    CHECK(cudaFree(d_A)),   CHECK(cudaFree(d_B)),   CHECK(cudaFree(d_C));
    CHECK(cudaFree(d_Aa)),   CHECK(cudaFree(d_Bb)),   CHECK(cudaFree(d_Cc));
